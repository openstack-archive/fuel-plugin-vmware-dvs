...
def check_connection_vms(os_conn, fip, remote, command='pingv4',
    ...
    logger.info(ip_pair)
    for ip in ip_pair:
        logger.info("Connect to VM {0}".format(ip))
        command_result = os_conn.execute_through_host(
            remote, ip,
            commands[command].format(ip_pair[ip]), instance_creds)

        assert_true(
            result_of_command == command_result['exit_code'],
            " Command {0} from Vm {1},"
            " executed with code {2}".format(
                commands[command].format(ip_pair[ip]),
                ip, command_result)
        )


def create_and_assign_floating_ips(os_conn, srv_list):
    """Create Vms on available hypervisors.
