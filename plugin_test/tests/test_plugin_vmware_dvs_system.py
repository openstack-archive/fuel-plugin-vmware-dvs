...
class TestDVSSystem(TestBasic):
    ...
    def dvs_vcenter_multiple_nics(self):
        ...
        for ip in ips[net_1_name]['ips']:
            ...
            logger.info('Execute for ip {}'.format(ip))
            cmd = (
            ...
            result = openstack.remote_execute_command(access_point_ip, ip, cmd)
            assert_true(result['exit_code'] == 0,
                        'Changing /etc/network/interfaces has failed')

        map((lambda inst: inst.reboot()), instances)
        openstack.verify_instance_state(os_conn, instances=instances)
