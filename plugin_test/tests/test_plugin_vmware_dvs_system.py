...
class TestDVSSystem(TestBasic):
    ...
    def dvs_vcenter_multiple_nics(self):
        ...
        ips = {}
        for net in networks:
            ips[net['name']] = map((lambda x: os_conn.get_nova_instance_ip(
                        x, net_name=net['name'])), instances)

        self.show_step(6)
        # Activate second interfaces on instances
        for ip in ips[self.net_data[0]['name']]:
            command = (
        ...
        self.show_step(7)
        for net in networks:
            ip_pair = {ip: [v for v in ips if v != ip] for ip in ips[net['name']]}
            openstack.check_connection_through_host(
