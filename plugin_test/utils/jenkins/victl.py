...
class Victl:
    ...
    def get_dc_object(self, datacenter):
        ...
        raise Exception("Can not find dc '{dc_name}'".format(datacenter))

    def get_hosts_in_cluster(self, dc, in_cluster):
        ...
        # raise Exception("Cluster '{cl_name}' is empty".format(in_cluster))
        print("Cluster '{cl_name}' is empty".format(in_cluster))
        return []
    ...
    def get_vds_object(self, dc, vds):
        ...
        raise Exception("dvSwitch '{vds}' not found".format(vds=vds))

    def get_hosts_in_vds(self, vds):
    ...
    def list_clusters(self, datacenter):
    ...
    def _exec_command(self, host, user, password, cmd):
        ...
        try:
            ...
            raise Exception('There is no valid connections')
        finally:
            if client:
                client.close()
        return out
    ...
    def restart_netcpad(self, host, user, password):
        ...
        print("Host '{host}', try restart netcpad".format(host=host))

        cmd = r"/etc/init.d/netcpad restart || /etc/init.d/netcpad start"
        self._exec_command(host, user, password, cmd)

    def check_portgroup_configured(self, datacenter, in_cluster, portgroup):
    ...
    def check_storage_configured(self, datacenter, in_cluster, datastore):
            ...
            if cluster.name == in_cluster:
                for esxi in cluster.host:
                    if datastore not in [ds.name for ds in esxi.datastore]:
                        err += "On esxi '{esxi}' datastore '{ds}' not " \
    ...
    def write_test_datastore(self, datacenter, datastore, host):
        ...
        if datastore not in [ds.name for ds in dc.datastore]:
            raise Exception("Datastore '{ds}' not found on '{dc}' "
    ...
    def get_cluster_object(self, cluster_name, datacenter):
        """Return cluster object with specified name"""
...
def cluster_list(args, inst):
    ...
    clusters = inst.list_clusters(args.datacenter)
...
def check_dvs_attached(args, inst):
    ...
    hosts_in_cluster = inst.get_hosts_in_cluster(dc, args.cluster)
    hosts_in_vds = inst.get_hosts_in_vds(vds)
    ...
    for hostname, host_nics in zip(hosts_in_cluster, nics):
        ...
        if extra_nic:
            print("Host '{host}' has extra nic '{nic}' attached to "
                  "dvSwitch '{vds}'".format(host=hostname,
                                            nic=','.join(extra_nic),
                                            vds=vds.name))
...
def datastore_list(args, inst):
    ...
    cluster = inst.get_cluster_object(args.cluster, args.datacenter)
    print("In cluster '{cl_name}'".format(cl_name=args.cluster))

    for esxi in cluster.host:
        print("  On esxi '{esxi}' datastores:".format(esxi=esxi.name))
