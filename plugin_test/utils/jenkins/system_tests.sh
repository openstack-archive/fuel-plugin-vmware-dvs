...
CheckVariables() {
  ...
  if [ -z "${WS_LOGIN}" ]; then
    echo "Error! WS_LOGIN is not set!"
    exit $NOWS_LOGIN
  fi
...
revert_ws() {
  for i in $1
  do
    vmrun -T ws-shared -h https://localhost:443/sdk -u $WS_LOGIN -p $WS_PASSSWORD listRegisteredVM | grep -q $i || { echo "VM $i does not exist"; continue; }
    echo "vmrun: reverting $i to $WS_SNAPSHOT"
    vmrun -T ws-shared -h https://localhost:443/sdk -u $WS_LOGIN -p $WS_PASSSWORD revertToSnapshot "[standard] $i/$i.vmx" $WS_SNAPSHOT || { echo "Error: revert of $i falied";  return 1; }
    echo "vmrun: starting $i"
    vmrun -T ws-shared -h https://localhost:443/sdk -u $WS_LOGIN -p $WS_PASSSWORD start "[standard] $i/$i.vmx" || { echo "Error: $i failed to start";  return 1; }
  done
}
