...
)
{
  ...
  exec {'install-suds':
    command => 'pip install -q suds==0.4 --source git+git://github.com/yunesj/suds.git#egg=suds',
  } ->
    exec {'install-dvs':
    command => 'pip install -q --upgrade git+git://github.com/Mirantis/vmware-dvs.git@stable/kilo#egg=mech-vmware-dvs',
  }
